 ...
 * fetched client-side — a search widget hydrates from it, but the full list a crawler sees is
 * already here on first load. Loose about keys this Source has no use for (`id`, `slug`,
 * `founded`, `status`, `location`, the thumbnail fields): the page carries more than a Profile
 * needs, and a new one is not a reason to stop ingesting.
 */
