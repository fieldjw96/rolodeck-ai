 ...
 * project and where to find it, rather than pretending to be a visitor's browser. None of
 * these four sites requires one — South Park Commons and AngelPad's `robots.txt` both allow an
 * unnamed client — but sending one anyway is what "honestly, at one request a second" means in
 * practice, not just in the rate limit below.
 *
 * Rate-limited to one request a second per host, per Ticket #50's Acceptance Criteria. Each
 * page here is fetched once, so the limit is never actually reached today; it exists so a
 * later batch page needing more than one request per site inherits the limit rather than
